...
public value class LowResCoord(public val packed: Int) {
    ...
    public val y: Int get() = packed and 0xFF

    public val level: Int get() = (packed shr 16) and 0x3

    public constructor(x: Int, y: Int, level: Int = 0) : this(
        (y and 0xFF) or ((x and 0xFF) shl 8) or ((level and 0x3) shl 16)
    )

    public fun toHighRes(): HighResCoord {
        return HighResCoord((x shl 13) and 0x3FFF, y shl 13, level)
    }
    ...
    public operator fun minus(other: LowResCoord): LowResCoord {
        return LowResCoord(x - other.x, y - other.y, level - other.level)
    }
    ...
    public override fun toString(): String {
        return "LowResCoord(x=$x, y=$y, level=$level)"
    }
